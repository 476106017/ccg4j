...
var cardHtml = function(card){
    ...
    if (card.TYPE === 'FOLLOW' && card.equipment) {
        equipmentIcon = '<div class="equipment-indicator" title="装备: ' + (card.equipment.name || '未知装备') + '">⚔️</div>';
    }
    ...
    return `
        <div class="card col-sm-6 col-md-4 col-lg-2 id-${card.id} ${card.TYPE} ${typeClass} ${card.canAttack?'canAttack':''} ${card.canDash?'canDash':''} ${card.rarity||'BRONZE'}" data-keywords='${JSON.stringify(card.keywords||[])}' data-mark='${(card.mark||"").replace(/'/g,"\\'")}' data-equipment='${card.equipment ? JSON.stringify(card.equipment).replace(/'/g,"\\'") : ""}'>
            ...
            <div ${card.TYPE!="FOLLOW"?"hidden":""}>
                ...
                <div class="health-bar-small">
                    <div class="health-bar-inner-small" style="width: ${Math.max(0, Math.min(100, (card.hp/card.maxHp*100 || 0)))}%;"></div>
                </div>
                ${equipmentIcon}
            </div>
            <div ${card.TYPE!="EQUIP"?"hidden":""}>
