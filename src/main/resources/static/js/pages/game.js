...
var drawBoard = function () {
  ...
  function generateStatusWarning(leaderStatuses, playerType) {
    ...
    var tooltipContent = leaderStatuses
      .map(function (status) {
      ...
      .join(" | ");
    return (
      '<div class="leader-status-warning" data-player="' +
      playerType +
      '" title="' +
      tooltipContent.replace(/"/g, "&quot;").replace(/'/g, "&#39;") +
      '">⚠️</div>'
    );
  }

  var enemyStatusWarning = generateStatusWarning(
    boardInfo.enemy.leaderStatuses,
    "enemy"
  );
  var myStatusWarning = generateStatusWarning(
    boardInfo.me.leaderStatuses,
    "me"
  );
      ...
      .on("click", function () {
        ...
        var statuses =
          playerType === "enemy"
            ? boardInfo.enemy.leaderStatuses
            : boardInfo.me.leaderStatuses;
        showLeaderStatusModal(
          statuses,
          playerType === "enemy" ? boardInfo.enemy.name : boardInfo.me.name
        );
      });
  }, 100);
  ...
  $("#enemy-info").html(
    '<span class="leader-health" title="' +
      boardInfo.enemy.hp +
      "/" +
      boardInfo.enemy.hpMax +
      '">' +
      boardInfo.enemy.hp +
      "</span>" +
      "<p title='超抽效果：" +
      boardInfo.enemy.leader.overDrawMark +
      "'>🗃️" +
      boardInfo.enemy.deckCount +
      "</p>💀" +
      boardInfo.enemy.graveyardCount
  );
  $("#my-info").html(
    "💀" +
      boardInfo.me.graveyardCount +
      "<br/><p title='超抽效果：" +
      boardInfo.me.leader.overDrawMark +
      "'>🗃️" +
      boardInfo.me.deckCount +
      "</p>" +
      '<span class="leader-health" title="' +
      boardInfo.me.hp +
      "/" +
  ...
  $("#enemy-info-detail").html(
    enemyStatusWarning +
      "<p class='skill' title='" +
      boardInfo.enemy.leader.skillMark +
      "'>" +
      boardInfo.enemy.leader.skillName +
      "(" +
      boardInfo.enemy.leader.skillCost +
      ")</p>" +
      "<p title='" +
      boardInfo.enemy.leader.mark +
      "'>主战者：" +
      boardInfo.enemy.leader.name +
      "</p>"
  );
  ...
  $("#my-info-detail").html(
    "<p title='" +
      boardInfo.me.leader.mark +
      "'>主战者：" +
      boardInfo.me.leader.name +
      "</p>" +
      "<p class='skill' title='" +
      boardInfo.me.leader.skillMark +
      "'>" +
      boardInfo.me.leader.skillName +
      "(" +
      boardInfo.me.leader.skillCost +
      ")</p>" +
      myStatusWarning
  );
  if (boardInfo.me.leader.canUseSkill) {
    $("#my-info-detail .skill").addClass("canUse");
    $("#my-info-detail .skill")
      .unbind()
      .click(function () {
        setTimeout("websocket.send('skill')", 500);
      });
  } else {
  ...
  // 使用宝石（Gem）显示 PP：纯CSS样式
  function renderGems(ppNum, ppMax, isEnemy) {
    var html = "";
    for (var i = 0; i < ppMax; i++) {
      if (i < ppNum) html += '<span class="gem"></span>';
      else html += '<span class="gem used"></span>';
    }
    return '<div class="pp-gems">' + html + "</div>";
  }

  $(".enemy-pp-num").html(
    renderGems(boardInfo.enemy.ppNum, boardInfo.enemy.ppMax, true)
  );
  $(".my-pp-num").html(
    renderGems(boardInfo.me.ppNum, boardInfo.me.ppMax, false)
  );
  ...
  $(".card").on("dragstart", function (e) {
    ...
    // 传递来源与 index
    var from = $card.closest("#my-battlefield").length
      ? "my-battlefield"
      : $card.closest("#my-hand").length
      ? "my-hand"
      : "other";
  ...
  $("#enemy-battlefield, #enemy-info, #my-battlefield, #my-info").off(
    "dragover drop"
  );
  $("#enemy-battlefield, #enemy-info, #my-battlefield, #my-info").on(
    "dragover",
    function (e) {
      e.preventDefault();
    }
  ...
  $("#enemy-battlefield, #enemy-info, #my-battlefield, #my-info").on(
    ...
    function (e) {
      ...
      if (data && data.from === "my-battlefield") {
        ...
        if (
          $targetCard.length &&
          $targetCard.closest("#enemy-battlefield").length
        ) {
      ...
      // 如果来源是手牌，尝试直接发起带目标的 play 指令（优先使用目标卡的真实 id）
        ...
        if (
          ...
          var targetId = idClass
            ? idClass.substring(3)
            : $targetCard.index() + 1;
          setTimeout(function () {
            websocket.send("play::" + handIdx + " " + targetId);
          }, 100);
        } else if ($(this).is("#enemy-info")) {
          setTimeout(function () {
            websocket.send("play::" + handIdx + " 0");
          }, 100);
        }
      }
      // 清理拖拽样式
        ...
        $("#enemy-battlefield .card").each((i, card) => {
          if ($(card).hasClass("FOLLOW")) {
            ...
            $(card)
              .unbind()
              .click(() => {
                var $attackerCard = $("#my-battlefield .card").eq(
                  targetMsg - 1
                );
                $attackerCard.addClass("card-attack-animate");
                setTimeout(function () {
  ...
  $(".leader-affecting-indicator")
    .on("mouseenter", function (e) {
      var $container = $(this).closest("#my-info, #enemy-info");
      var isEnemy = $container.attr("id") === "enemy-info";
      var affectingCards = isEnemy
        ? boardInfo.enemy.leader.affectingCards
        : boardInfo.me.leader.affectingCards;
...
function showCardsHover(cards, x, y) {
  ...
  var $hoverDiv = $('<div class="card-hover-display"></div>');
  var html =
    '<div style="display: flex; flex-wrap: wrap; gap: 10px; background: rgba(0,0,0,0.8); padding: 15px; border-radius: 12px;">';
  cards.forEach((card) => {
    html += cardHtml(card);
  });
  html += "</div>";
...
function mnyAlert(type, msg, time = 1000) {
  // 将消息加入队列
...
var pendingAutoMatchMode = hasBoardUi
  ? localStorage.getItem("pendingMatchMode")
  : null;
...
function initWebSocket(userName) {
  if ($.trim(userName)) {
    ...
    else websocket = new WebSocket("ws://8.216.80.59/api/" + userName);

    $("username").html(userName);
    console.log(
      "征集有趣的自定义卡牌、主战者、玩法、卡面。联系方式：（Bilibili）漆黑Ganker"
    );
    console.log(
      "如果你是软件开发人员，欢迎你贡献代码！项目地址：https://github.com/476106017/ccg4j"
    );
    ...
    websocket.onmessage = function (msg) {
      ...
      // Dispatch event for social features
      ...
      switch (data.channel) {
        ...
        case "search_results":
          ...
          obj.forEach((card) => {
            ...
            $card.click(function () {
              let cardCount = myDeck
                ? myDeck.split("#").filter((name) => name === card.name).length
                : 0;
              let totalCards = myDeck
                ? myDeck.split("#").filter((name) => name.length > 0).length
                : 0;
        ...
        case "myDeck":
          ...
          obj.deck.forEach((card, index) => {
            ...
            // 添加删除按钮
            const $deleteBtn = $(
              '<button type="button" class="btn btn-danger btn-sm delete-card">×</button>'
            );
        ...
        case "presetDeck":
          ...
          obj.forEach((deck) => {
            $("#deck-preset").append(
              '<button type="button" class="btn btn-outline-dark" data-dismiss="modal" onclick="websocket.send(\'usedeck::' +
                deck.name +
                "');\">" +
                deck.name +
                "</button>"
          ...
          try {
            if (prevBoard) {
              ...
              if (
                prevBoard.me &&
                boardInfo.me &&
                prevBoard.me.hp > boardInfo.me.hp
              ) {
              ...
              if (
                prevBoard.enemy &&
                boardInfo.enemy &&
                prevBoard.enemy.hp > boardInfo.enemy.hp
              ) {
...
function adjustDescriptionFont(containerSelector) {
  ...
  var $ps = containerSelector
    ? $(containerSelector).find(".description p")
    : $(".description p");
  $ps.each((i, p) => {
    var $p = $(p);
    ...
    try {
      while (
        $p[0].scrollHeight > $desc[0].clientHeight &&
        parseFloat($p.css("font-size")) > minFont
      ) {
...
$(document).on("mouseenter", ".card", function (e) {
  ...
  if (kw && kw.length > 0 && keywordsData) {
    ...
    kw.forEach((k) => {
      var keywordInfo = keywordsData.find(
        (x) => x.key === k || x.key === k + ""
      );
      var desc = keywordInfo ? keywordInfo.desc : "（无描述）";
      leftHtml +=
        '<div class="keyword-inline"><b>' + k + ":</b> " + desc + "</div>";
    });
  }
  ...
  if (/瞬念召唤|瞬念/.test(mark)) {
    triggers.push({
      key: "瞬念召唤",
      desc: "当此卡符合指定条件时，从牌堆召唤并触发效果",
    });
  }
  if (/揭示/.test(mark)) {
    triggers.push({
      key: "揭示",
      desc: "当此卡符合指定条件时，从牌堆抽到手中并触发效果",
    });
  }
  if (/腐蚀/.test(mark)) {
    triggers.push({
      key: "腐蚀",
      desc: "当使用费用高于此卡片的其他卡片时触发的效果",
    });
  }
  if (/离场时|不在场时/.test(mark)) {
    triggers.push({
      key: "离场时",
      desc: "当该随从离开战场（返回手牌、被除外、被破坏等）时触发的效果",
    });
  }
  if (/超杀|超额杀死|超杀时/.test(mark)) {
    triggers.push({
      key: "超杀",
      desc: "当对手单位被造成的伤害超过其剩余生命时触发的额外效果",
    });
  }
  ...
  if (leftHtml || rightHtml) {
    ...
    if (leftHtml) {
      var $leftTooltip = $(
        '<div class="card-tooltip-left">' + leftHtml + "</div>"
      ).appendTo($target);
    ...
    if (rightHtml) {
      var $rightTooltip = $(
        '<div class="card-tooltip-right">' + rightHtml + "</div>"
      ).appendTo($target);
      ...
      if ($target.is("body")) {
        ...
        $rightTooltip
          .css({
            position: "fixed",
            top:
              viewportTopR + cardHeight / 2 - $rightTooltip.outerHeight() / 2,
